...
class LoraModel(PeftModel):
    def __init__(self, config: Config, model):
        ...
        if lora_args.is_reload_trained_params:
            super().from_pretrained(model, config.save_lora_path, torch_dtype=torch.float16)
...
class Model(torch.nn.Module):
    def __init__(self, config: Config):
        super().__init__()
        model = AutoModelForCausalLM.from_pretrained(config.gpt_model, torch_dtype=torch.float16).to(config.device).eval()

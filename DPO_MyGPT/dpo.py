...
class DPO:
    ...
    def train(self, inputs_ids, attention_mask, ref_logits, labels_mask):
        # import ipdb
        # ipdb.set_trace()
        ref_token_logps = self.probs_from_logits(torch.tensor(ref_logits)[:, :-1, :], inputs_ids[:, 1:1024])
        ref_logps = self.filter_mask(ref_token_logps, labels_mask[:, :1024])

        # 一次数据多训练几次，这样reference只用计算一次
        for dpo_epoch in range(self.dpo_epochs):
            # 计算策略模型的logps
            logits = self.model(inputs_ids, attention_mask)
            policy_token_logps = self.probs_from_logits(logits[:, :-1, :], inputs_ids[:, 1:1024])
            policy_logps = self.filter_mask(policy_token_logps, labels_mask[:, :1024])

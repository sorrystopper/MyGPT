...
class TrainDpo:
    def __init__(self):
        ...
        self.data_loader = DataLoader(dataset, batch_size=self.config.batch_size, shuffle=False,
                                      collate_fn=dataset.collate_fn)
        self.dpo = DPO(self.model, self.model_opt, self.config)

    def train_dpo(self):
        for epoch in range(self.config.epochs):
            for batch_data in tqdm(self.data_loader, leave=False):
                for key in ["inputs_ids", "inputs_masks"]:
    ...
    def save_model(self):
        torch.save({'model': self.model.model.state_dict()},"/home/xli/skw/MyGPT/DPO_MyGPT/ckpt/Qwen1.5-0.5B-Chat/dpo_model")
